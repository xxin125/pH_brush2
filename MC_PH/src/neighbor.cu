...
CSR copy_csr_from_device(const DeviceCSR& dev){
    CSR host;
    host.head.resize(dev.head.size());
    host.list.resize(dev.list.size());
    thrust::copy(dev.head.begin(), dev.head.end(), host.head.begin());
    thrust::copy(dev.list.begin(), dev.list.end(), host.list.begin());
    return host;
}

HostNeigh build_host_symmetric(const CSR& half, int N){
    if (static_cast<int>(half.head.size()) != N + 1){
        throw std::runtime_error("build_host_symmetric: CSR size mismatch");
    }
    HostNeigh out;
    out.nb.assign(N, {});
    for (int i = 0; i < N; ++i){
        for (int kk = half.head[i]; kk < half.head[i + 1]; ++kk){
            int j = half.list[kk];
            if (j < 0 || j >= N){
                throw std::runtime_error("build_host_symmetric: neighbor index out of range");
            }
            out.nb[i].push_back(j);
            out.nb[j].push_back(i);
        }
    }
    return out;
}
