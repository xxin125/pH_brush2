...
Params read_params(const std::string& path, int ntypes){
    ...
    while (std::getline(in, line)){
        ...
        if (!key.empty()){
            ...
            } else if (key=="calibration_offset" || key=="offset") {
                p.calibration_offset = parse_real(val);
            } 
            // ====== constant-pH parameters ======
            else if (key=="pH" || key=="ph") {
                p.pH = parse_real(val);
            } else if (key=="pKa_NH" || key=="pka_nh" || key=="pKa") {
                p.pKa_NH = parse_real(val);
            } else if (key=="cph_attempts" || key=="const_pH_attempts") {
                p.cph_attempts = std::stoi(val);
            } else if (key=="rc_move" || key=="cutoff_move_nm") {
                p.rc_move = parse_real(val);
            }
    ...
    // Defaults for constant-pH SGMC
    if (p.pH == static_cast<real>(0.0)) p.pH = static_cast<real>(7.0);
    if (p.pKa_NH == static_cast<real>(0.0)) p.pKa_NH = static_cast<real>(7.5);
    if (p.cph_attempts <= 0) p.cph_attempts = 100000;
